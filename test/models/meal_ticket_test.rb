...
class MealTicketTest < ActiveSupport::TestCase
  ...
  test "should be valid with required attributes" do
    ticket = MealTicket.new(child: @child, date: Date.today, bought: true)
    assert ticket.valid?
  end

  test "date should be present" do
    ticket = MealTicket.new(child: @child, bought: true)
    assert_not ticket.valid?
    assert ticket.errors[:date].any?
  end

  test "bought should be present" do
    ticket = MealTicket.new(child: @child, date: Date.today)
    assert_not ticket.valid?
  ...
  test "should have unique date per child" do
    MealTicket.create!(child: @child, date: Date.today, bought: true)
    duplicate = MealTicket.new(child: @child, date: Date.today, bought: false)
    assert_not duplicate.valid?
    assert duplicate.errors[:date].any?
  end

  test "different children can have same date" do
    other_child = @user.children.create!(giae_username: "other", giae_password: "pass")
    MealTicket.create!(child: @child, date: Date.today, bought: true)
    other_ticket = MealTicket.new(child: other_child, date: Date.today, bought: false)
    assert other_ticket.valid?
  end

  test "syncs user_id from child on create" do
    ticket = MealTicket.create!(child: @child, date: Date.today, bought: true)
    assert_equal @user, ticket.user
  end

  test "scope bought_tickets should return bought tickets" do
    MealTicket.create!(child: @child, date: Date.today, bought: true)
    MealTicket.create!(child: @child, date: Date.today + 1.day, bought: false)

    assert_equal 1, @child.meal_tickets.bought_tickets.count
    assert_equal Date.today, @child.meal_tickets.bought_tickets.first.date
  end
  ...
  test "scope not_bought_tickets should return not bought tickets" do
    ...
    assert_equal 1, @child.meal_tickets.not_bought_tickets.count
    assert_equal Date.today + 1.day, @child.meal_tickets.not_bought_tickets.first.date
  end
  ...
  test "dish_type_icon returns meat icon for meat dishes" do
    ticket = MealTicket.new(child: @child, date: Date.today, bought: true, dish_type: "meat")
  ...
  test "dish_type_icon returns fish icon for fish dishes" do
    ticket = MealTicket.new(child: @child, date: Date.today, bought: true, dish_type: "fish")
    assert ticket.dish_type_icon.present?
  ...
  test "dish_type_icon returns nil for unknown dish type" do
    ticket = MealTicket.new(child: @child, date: Date.today, bought: true, dish_type: nil)
  ...
  test "child can have multiple tickets" do
    MealTicket.create!(child: @child, date: Date.today, bought: true)
    MealTicket.create!(child: @child, date: Date.today + 1.day, bought: true)
    MealTicket.create!(child: @child, date: Date.today + 2.days, bought: false)

    assert_equal 3, @child.meal_tickets.count
  end

  test "bought scope excludes not bought tickets" do
    bought = MealTicket.create!(child: @child, date: Date.today, bought: true)
    MealTicket.create!(child: @child, date: Date.today + 1.day, bought: false)
  ...
  test "not_bought scope excludes bought tickets" do
    MealTicket.create!(child: @child, date: Date.today, bought: true)
    not_bought = MealTicket.create!(child: @child, date: Date.today + 1.day, bought: false)
  ...
  test "can update dish_type" do
    ticket = MealTicket.create!(child: @child, date: Date.today, bought: true, dish_type: "meat")
